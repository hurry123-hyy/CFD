...
class Point {
   ...
   public:
    void Get_Point_Coord(double& xNode, double& yNode) {
        xNode = this->xPoint;
        yNode = this->yPoint;
    }
    void Set_Point_Coord(double xNode, double yNode) {
        this->xPoint = xNode;
        this->yPoint = yNode;
    }
};
...
class Structured_Mesh {
   ...
   public:
    vector<vector<Point> >& Get_Grid_Points() {
        return grid_points;
    }
    VInt2D& Get_Marker_Q() {
        return marker_q;
    ...
    void Set_Dxdy(double dx, double dy) {
        this->dx = dx;
        this->dy = dy;
    }
    void Set_Range(int ist, int ied, int jst, int jed);
    void Set_Num_Ghost_Point(int num_ghost_point) {
        this->num_ghost_point = num_ghost_point;
    }
