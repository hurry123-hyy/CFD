...
Residual::Residual() {
    res_L1.resize(num_of_primitive_vars);
    res_L2.resize(num_of_primitive_vars);
    res_Loo.resize(num_of_primitive_vars);
    Allocate_2D_Vector(max_index, num_of_primitive_vars, 2);
...
void Residual::OutputResidual() {
    ...
    cout << setprecision(5);
    cout << current_step << "\t        " << res_L2[IR] << "\t" << res_L2[IU] << "\t" << res_L2[IV] << "\t" << res_L2[IP]
         << endl;

    if (res_Loo[IR] < converge_criterion && res_Loo[IU] < converge_criterion && res_Loo[IV] < converge_criterion &&
...
void Output_Flowfield() {
    ...
    cout << "dumping flowfield..."
         << "\tIter = " << current_step << "\tphysical_time = " << physical_time
         << "\ttime_elapsed  = " << static_cast<double>(nowTime - lastTime) / GetClockTicksPerSecond() << " seconds"
         << endl
         << endl;
    // lastTime = nowTime;
    ...
    if (!file.is_open()) {
        cerr << "Error: Could not open file " << tec_file_name << endl;
        return;
    }

    file << "VARIABLES = \"x\", \"y\", \"rho\", \"u\", \"v\", \"p\", \"m\", \"iblank\" " << endl;
    file << "ZONE T = \"Zone 1\"" << endl;
    file << "I = " << num_grid_point_x << "  J = " << num_grid_point_y << "  K =1"
