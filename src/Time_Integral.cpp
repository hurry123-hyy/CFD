...
void Update_Flowfield_X(int iStage) {
    ...
    VDouble qPrimitive0(num_of_primitive_vars);
    VDouble qPrimitive1(num_of_primitive_vars);
    VDouble qConservative0(num_of_primitive_vars);
        ...
        // for (int i = ist; i <= ied; i++)
        {
            for (int j = jst; j <= jed; j++) {
                ...
                for (int iVar = 0; iVar < num_of_primitive_vars; iVar++) {
                    qConservative1[iVar] = RK_Coeff_a * qConservative0[iVar] + RK_Coeff_b * qConservative1[iVar] +
                                           RK_Coeff_c * time_step * rhsVector[iVar];
                }
...
void SolutionFix(VDouble& primitiveVector, int iCell, int jCell) {
    ...
    for (int iEquation = 0; iEquation < num_of_primitive_vars; ++iEquation) {
